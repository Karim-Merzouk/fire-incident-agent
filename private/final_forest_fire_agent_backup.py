...
try:
    ...
    print(f"⚠️  Google AI import error: {e}")
...
try:
    ...
    print(f"⚠️  Requests import error: {e}")
...
class ForestFireAgent:
    ...
    def __init__(self, api_key: str = None):
        ...
        self.agent = None
        self.gemini_model = None  # Always initialize this attribute
        self.conversation_history = []
        
        # Try to initialize with direct Google AI first
        if GOOGLE_AI_AVAILABLE and self.api_key:
            try:
                ...
                if REQUESTS_AVAILABLE:
                    ...
                    self.gemini_model = "direct_api"
                elif AGNO_AVAILABLE:
                    self._initialize_agent()
                else:
                    print("⚠️  Running in enhanced fallback mode")
        elif REQUESTS_AVAILABLE and self.api_key:
            print("🔄 Using direct Gemini API approach...")
            self.gemini_model = "direct_api"
        elif AGNO_AVAILABLE and self.api_key:
            self._initialize_agent()
        else:
            print("⚠️  Running in enhanced fallback mode")
    ...
    def _initialize_google_ai(self):
        ...
        try:
            # Import and configure within the method to avoid scope issues
...
"""
    ...
    def query(self, user_input: str) -> str:
        ...
        try:
            ...
            elif self.agent and AGNO_AVAILABLE:
    ...
    def _query_google_ai(self, user_input: str) -> str:
        ...
        try:
            ...
            # If using direct API approach
            if self.gemini_model == "direct_api":
                return self._query_gemini_direct_api(user_input)
...
Provide a professional, urgent but calm response with:
- Use proper Markdown formatting (##, ###, **, *, bullets)
- Include relevant fire statistics and emergency data
- Focus on life safety and emergency management
- Use fire/emergency emojis for visual clarity
...
RESPONSE:"""
            ...
            return f"⚠️ **AI API temporarily unavailable. Using local emergency data:**\n\n" + self._fallback_response(user_input)
